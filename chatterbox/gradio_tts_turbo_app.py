...
log_filename = f"c:/esp32-project/logs/chatterbox_logs/chatterbox_debug_{timestamp}.log"
...
# Robust TeeLogger from cli_demo.py
...
def generate(
    ...
    # If seed is 0, pick a random one and log it
    if seed_num == 0:
        seed_num = random.randint(1, 999999)
        print(f"Random seed selected: {seed_num}")
...
with gr.Blocks(title="Chatterbox Turbo", css=CUSTOM_CSS) as demo:
    ...
    # Remove model_state as we use global MODEL
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            with gr.Accordion("Advanced Options", open=True): # Open by default to show seed
                seed_num = gr.Number(value=42, label="Random seed (0 for random)", precision=0)
