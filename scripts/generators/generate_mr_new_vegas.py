"""Generate broadcast scripts for Mr. New Vegas"""
import sys
...
# Initialize engine
engine = BroadcastEngine('Mr. New Vegas (2281, Mojave)', enable_validation=True)
...
# Save to files
output_dir = Path('output/scripts/pending_review/Mr. New Vegas')
output_dir.mkdir(parents=True, exist_ok=True)

for i, seg in enumerate(segments):
    hour = seg['metadata']['hour']
    segment_type = seg['segment_type'].replace('_', '-').title()
    filename = f'2026-01-17_{hour:02d}{i:02d}00_Mr. New Vegas_{segment_type}.txt'
...
print(f'\n✅ Generated {len(segments)} Mr. New Vegas scripts in {output_dir}')
